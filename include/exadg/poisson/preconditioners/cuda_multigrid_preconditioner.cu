  ...
  unsigned int const                     h_level)
{
  ...
  matrix_free_data.append_mapping_flags(
    ExaDG::Poisson::CUDAWrappers::Operators::LaplaceKernel<dim, Number>::get_mapping_flags(
...
{
  AssertThrow(false, dealii::ExcNotImplemented("TODO:"));
...
template<int dim, typename Number, int n_components>
std::shared_ptr<ExaDG::Poisson::CUDAWrappers::LaplaceOperator<
  dim,
...
{
template void
Vector<double, ::dealii::MemorySpace::CUDA>::copy_locally_owned_data_from<double>(
  const Vector<double, ::dealii::MemorySpace::CUDA> &);

template void
Vector<float, ::dealii::MemorySpace::CUDA>::copy_locally_owned_data_from<float>(
