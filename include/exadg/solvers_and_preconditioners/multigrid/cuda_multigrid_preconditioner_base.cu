...
#include <exadg/solvers_and_preconditioners/multigrid/cuda_multigrid_algorithm.h>
#include <exadg/solvers_and_preconditioners/multigrid/cuda_multigrid_preconditioner_base.h>
...
#include <exadg/solvers_and_preconditioners/multigrid/transfers/cuda_mg_transfer_global_refinement.h>
...
MultigridPreconditionerBase<dim, Number>::initialize_matrix_free()
{
  ...
  for(unsigned int level = coarse_level; level <= fine_level; level++)
  {
    ...
    matrix_free_objects[level] =
      std::make_shared<ExaDG::CUDAWrappers::MatrixFree<dim, MultigridNumber>>();

    auto const & mg_level_info = level_info[level];

    dealii::IteratorFilters::LocallyOwnedLevelCell locally_owned_level_cell_filter;
    matrix_free_objects[level]->reinit(
      get_mapping(mg_level_info.h_level()),
      *matrix_free_data_objects[level]->get_dof_handler_vector()[0],
      *matrix_free_data_objects[level]->get_constraint_vector()[0],
      matrix_free_data_objects[level]->get_quadrature_vector()[0],
...
{
  AssertThrow(false, dealii::ExcMessage("TODO: update_matrix_free()"));
  ...
  unsigned int const                                                  dof_index)
{
  ...
  // if(data.use_global_coarsening)
  // {
  //   auto tmp = std::make_shared<MGTransferGlobalCoarsening<dim, MultigridNumber,
  //   VectorTypeMG>>();
  //
