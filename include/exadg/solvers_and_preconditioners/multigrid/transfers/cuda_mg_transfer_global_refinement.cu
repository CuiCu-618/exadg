  ...
  unsigned int const                                                          dof_handler_index)
{
  ...
  int const n_components = mg_matrixfree[max_level]->get_dof_handler().get_fe().n_components();
  ...
  for(unsigned int global_level = min_level; global_level <= max_level; global_level++)
  {
    ...
    auto const &       fe         = matrixfree->get_dof_handler().get_fe();
  ...
  for(auto deg : p_levels)
  {
    if(map_global_level_to_h_levels[deg].size() > 1)
    {
      ...
      std::shared_ptr<MGTransferH<dim, Number>> transfer(
        new MGTransferH<dim, Number>(map_global_level_to_h_levels[deg],
      ...
      transfer->build(mg_matrixfree[global_level]->get_dof_handler());
...
typedef dealii::LinearAlgebra::distributed::Vector<float, dealii::MemorySpace::CUDA>
  VectorTypeFloat;
